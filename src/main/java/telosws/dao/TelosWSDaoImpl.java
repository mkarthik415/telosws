...
public class TelosWSDaoImpl implements TelosWSDao, ServletContextAware {
    ...
    @Override
    public List<TouchClients> findClientByName(final String name) {

        List<TouchClients> finaleClient = new ArrayList<TouchClients>();
        ...
        try {
            ...
            for (Clients clients : returnClients) {
                TouchClients touchClients = new TouchClients(clients.getId(), clients.getClientName(), clients.getDepartment(), clients.getTotalPremiumAmount(), clients.getIndustry(), clients.getPolicyEndDate());
                finaleClient.add(touchClients);
            }
...
//                FileUtils.deleteQuietly(tempFile);
                for (Document file : returnDocuments) {
                    if (!StringUtils.containsIgnoreCase(file.getFileName(), "MANDATE")) {
                        ...
                        out.write(buff);
                        out.close();
                        file.setUrl("https://connect2telos.com/telosws/" + file.getFileName());
                        file.setScanned(null);
                    }
                }
            }
